...
def checkMPI():
  ...
  try:
      ...
      raise argparse.ArgumentTypeError('mpirun was not found')
...
def fftnsi(seqFile,alnFile,thread,mIterLong,log,cDir,tName,zName):
  ...
  try:
    subprocess.check_call(cl, shell=True, stdout=lh, stderr=lh, cwd=tName)
  except subprocess.CalledProcessError as e:
    print(e)
    cZip(cDir,tName,zName)
...
def alnFullSequenceClusters(nClusters,thread,mIterL,cDir,tName,zName):
  ...
  aln = lambda i : ('grp.' + str(i) + '.fas', 'grp.' + str(i) + '.aln')

  to_run_tuples = list(map(aln, range(nClusters)))
  to_run = list(filter(lambda x : len(list(SeqIO.parse(os.path.join(tName, str(x[0])),'fasta'))) > 1, to_run_tuples))
  to_copy = list(filter(lambda x : len(list(SeqIO.parse(os.path.join(tName, str(x[0])),'fasta'))) <= 1, to_run_tuples))
  ...
  if len(to_run):
    ...
    joblib.Parallel(n_jobs=num_parallel_jobs)(joblib.delayed(fftnsi)(x[0],x[1],num_threads_per_job,mIterL,log,cDir,tName,zName)  for x in to_run)

  if len(to_copy):
    num_parallel_jobs = math.ceil(len(to_copy)/thread) if nClusters < thread else thread
    joblib.Parallel(n_jobs=num_parallel_jobs)(joblib.delayed(shutil.copyfile)(os.path.join(tName, x[0]),os.path.join(tName, x[1])) for x in to_copy)
...
def getArguments():
  ...
  parser.add_argument('--mpi', required=False, action='store_true', help="Run in mpi environment")
